...
class TripletLoss(nn.Module):
    ...
    def forward(self, img_embs, txt_embs, img_r, txt_r, img_query=None, txt_query=None):
        ...
        img_embs = img_embs.reshape(-1, img_embs.shape[-1]) # (4bs,dim)
        txt_embs = txt_embs.reshape(-1, txt_embs.shape[-1]) # (20bs,dim)
        
        # #### 可视化一波
        self.count += 1
        if self.count % 1000 == 0 and self.count < 4000 or self.count % 5000 == 0: 
            cos_mat = cosine_sim(img_embs, txt_embs)
            cos_mat = cos_mat[0:cos_mat.shape[0]//32, 0:cos_mat.shape[1]//32]
            visualize_and_save(cos_mat, self.img_folder)
            print("loss image loaded!")


        # Compute setwise similarity with provided set similarity metric
        setwise_dist = self.similarity_fn(img_embs, txt_embs) # (4bs,(5*4)bs) #1019(bs,5bs)
        
        # generate mask based on the computed number of sets per images
        mask = (torch.eye(setwise_dist.shape[0]) > .5).cuda()
        mask = mask.view(-1, 1).repeat(1, self.set_per_img).reshape(setwise_dist.shape) # (bs,5bs) # 对角线正样本mask
