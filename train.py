...
def save_ckpt(state, is_best, filename='ckpt.pth.tar', prefix=''):
    ...
    # filename='checkpoint_{}.pth.tar'.format(epoch), prefix=opt.model_name + '/')
    if is_best:
    ...
    for itr, data in enumerate(data_loader):
        ...
        if torch.cuda.is_available():
            if 'butd' in args.data_name:
                img, txt, img_len, txt_len, recovery, _ = data # (200,34,2048),((vocab编号)1000,68), ((还真有img_len啊)200),(1000),((不清楚是什么东西)1000)
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            txt_emb = txt_emb[recovery] # recovery推测为 shuffle 的索引
            # print("img:",img_emb[0,0,0:8])
            # print("txt:",txt_emb[0,0,0:8])
            txt_r = txt_r[recovery] # 这之后 txt_r 就和 txt_emb一样了
        ...
        if torch.isnan(loss).any():
            print("!! NaN loss detected !!") # img_emb[:,?,:] # 在某个iter时突然全是nan了，前置iter都正常 # 图像和文本都有可能
            print(img_emb)
            print(txt_emb)
...
def main():
    ...
    # Construct the model
    if 'butd' in args.data_name:
        model = VSE(vocab.word2idx, args) # 1
    else:
        model = VSE(vocab.word2idx, args)
            
    if torch.cuda.is_available():
        if args.multi_gpu:  # 0 # 写的真细啊
    ...
    param_groups = [    # 各部分的学习率的配置
        {'params': list(set(module.img_enc.parameters()).difference(set(module.img_enc.cnn.parameters()))), 'lr': args.lr * args.img_pie_lr_scale}, # module.img_enc 中所有参数 除去 CNN 层的参数 # 单独设置图像编码器中非 CNN 的参数的学习率
        {'params': module.img_enc.cnn.parameters(), 'lr': args.lr}, #  CNN 层的参数使用默认的学习率
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            if epoch % args.eval_epoch == 0: # 1
                ...
                #### 1114防过拟合
                if rsum_val > bottom_score:
                    bottom_score = rsum_val
                    print(bottom_score)
